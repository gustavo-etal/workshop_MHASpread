...
population$S_bov_pop[population$node== 1804] <- 100                                # Set a initial susceptible population of 100 bovines
population$I_bov_pop[population$node== 1804] <- 10                                  # Infected 10 bovine in the farm id = 66
...
model_1     <- SEIR_model(population = population,                                                 #  Population database
                          ...
                          simulation_name = "scenario_1_test",                                     #  Simulation tag name
...
population <- update_population_in_model(original_population = MHASpread::population,
                                         model_1$populationdb )                                        # Complete  and update the  population database
...
plot_SEIR_animals(model_1, plot_suceptible_compartment = F, by_host = F)
## plot epidemic curve animal level by host
plot_SEIR_animals(model_1, plot_suceptible_compartment = F, by_host = T)
#plot the epidemic curve considering
plot_SEIR_farms(model_1)

# here will plot the control actions
myplots  <- plot_SEIR_vacc_depop_ani(modeloutput = model_1, summary_control = banco_summary)
